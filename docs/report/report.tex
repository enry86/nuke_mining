...
\frac{k}{2} $$ since is the maximum value of the combination $$
{{i}\choose{k}} $$
...
aborted its execution due to memory problems.
...
trained. Some tests on different datasets reveal that is not useful to
produce a bigger tree if data can not fit it.
...
that can through them. This lead to a compact tree
