...
\paragraph*{Evaluation System usage}
In order to achieve the comparison target, a useful BASH script is been
written to automate the task. The aim is to collect the results from
multiple running of the algorithms having in input both the same dataset to
train and the dataset to classify. Starting from 1\% to 100\%, with step of
ten percent, the dataset given in input to the script is cross validated
at several splitting sets.

The results are collected in a temporary file, as the outputting
classifications, and then several graphs are built using the stored data.
Thank to \emph{gnuplot} the graphs are easy to retrieve once the data file
and the configuration file are furnished as input.

Once the script ends its cycle, the local directory ``plot'' will contain
five graph: accuracy.ps, training.ps, classification.ps, nodes.ps and
memory.ps 

An example of use is the following:
\begin{verbatim}
./evaluation.sh training.arff class sepallength\
